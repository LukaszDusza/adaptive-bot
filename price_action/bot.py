...
class TradingBot:
    ...
    def _cancel_unfilled_limit_orders(self, reason: str = "position_closed"):
        ...
        for order_info in self.active_limit_orders[:]:
            try:
                order_id = order_info['order_id']
                resp = self.adapter.cancel_order(self.config.TICKER, order_id)
    ...
    def get_decision(self, df=None) -> str:
        ...
        try:
            ...
            if self.config.ICT_CONTEXT_MODE:
                ict_composite = last_row.get('ict_composite_score', 0.5)
                ...
                if (rsi > 60 and proba_buy > proba_sell) or (rsi < 40 and proba_sell > proba_buy):
                    regime_multiplier *= (1.0 - 0.1 * self.config.REGIME_SENSITIVITY)

                if atr_norm > 1.2 and volume_ratio > 1.3:
                    regime_multiplier *= (1.0 - 0.15 * self.config.REGIME_SENSITIVITY)

                if atr_norm < 0.7 or volume_ratio < 0.6:
                    regime_multiplier *= (1.0 - 0.05 * self.config.REGIME_SENSITIVITY)

                if (rsi > 75 or rsi < 25) and volume_ratio < 0.8:
                    regime_multiplier *= (1.0 + 0.10 * self.config.REGIME_SENSITIVITY)
            ...
            if self.config.RSI_REVERSAL_FILTER_ENABLED:
                rsi_current = last_row.get('rsi_14', 50)
            ...
            if self.config.LIMIT_ORDER_MODE:
                try:
                    ...
                    if existing_orders:
                        ...
                        for order in existing_orders[:3]:
                ...
                if self.config.DCA_ENABLED:
                    ...
                    furthest_level = dca_price
                    expected_avg_entry = dca_price
        ...
        try:
            ...
            if filled_orders:
                ...
                for retry in range(3):
                    time.sleep(0.5 if retry > 0 else 0)
                    position = self.adapter.get_position(self.config.TICKER)
                    if position:
                        break
    ...
    def _finalize_limit_order_entry(self, position: dict, filled_orders: list = None, remaining_orders_count: int = 0):
        ...
        try:
            ...
            if is_first_fill:
                ...
                decision = self.state['side'].upper()
                ...
                if decision == "LONG":
                    target_be_sl = actual_entry * (1 - 0.0006)
                else:
                    target_be_sl = actual_entry * (1 + 0.0006)
        ...
        try:
            ...
            while True:
                ...
                if self.active_limit_orders:
                    ...
                    sleep_time = self.config.LOOP_SLEEP_SECONDS
...
def launch_bot(args):
    ...
    config.DCA_ATR_MULTIPLIER = getattr(args, 'dca_atr_multiplier', 1.5)

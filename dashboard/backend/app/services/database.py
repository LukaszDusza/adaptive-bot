...
class TradeRepository:
    ...
    def get_trade(self, trade_id: str) -> Optional[TradeModel]:
        """Get trade by ID with eagerly loaded relationships"""
        with self.db.session_scope() as session:
            trade = (
                session.query(TradeModel)
                .options(joinedload(TradeModel.events))
                .options(joinedload(TradeModel.indicators))
    ...
    def get_all_trades(self, limit: int = None, ticker: str = None) -> List[TradeModel]:
        ...
        Returns:
            List of TradeModel instances with eagerly loaded relationships
        """
        with self.db.session_scope() as session:
            query = (
                session.query(TradeModel)
                .options(joinedload(TradeModel.events))
                .options(joinedload(TradeModel.indicators))
                .order_by(TradeModel.start_time.desc())
            )
            ...
            # Expunge from session to make them detached (no longer tied to session)
            for trade in trades:
                session.expunge(trade)
    ...
    def get_active_trades(self) -> List[TradeModel]:
        """Get all currently active trades with eagerly loaded relationships"""
        with self.db.session_scope() as session:
            trades = (
                session.query(TradeModel)
                .options(joinedload(TradeModel.events))
            ...
            # Expunge from session to make them detached
            for trade in trades:
                session.expunge(trade)
